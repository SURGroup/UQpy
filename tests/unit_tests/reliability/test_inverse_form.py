...
def test_no_seed(inverse_form):
    inverse_form.run()
    assert np.allclose(
        inverse_form.design_point_u, np.array([1.7367, 0.16376]), atol=1e-4
    )
...
def test_neither_tolerance():
    ...
    distributions = [Normal(loc=500, scale=100), Normal(loc=1_000, scale=100)]
    with pytest.raises(
        ValueError, match="UQpy: At least one tolerance .* must be provided"
    ):
...
def test_beta():
    ...
    inverse_form.run()
    assert np.allclose(
        inverse_form.design_point_u, np.array([1.7367, 0.16376]), atol=1e-3
    )
