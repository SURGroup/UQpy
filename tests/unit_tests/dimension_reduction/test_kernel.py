from UQpy.utilities import ProjectionKernel
from UQpy.utilities.GrassmannPoint import GrassmannPoint
from UQpy.dimension_reduction.grassmann_manifold.projections.SVDProjection import (
    SVDProjection,
)
from UQpy.utilities.kernels.grassmannian_kernels.BinetCauchyKernel import (
    BinetCauchyKernel,
)
...
def test_kernel_projection():
    ...
    xk = GrassmannPoint(
        np.array(
            [
                [-0.69535592, -0.0546034],
                [-0.34016974, -0.85332868],
    ...
    np.testing.assert_allclose(
        kernel,
        np.array([[2, 1.0063, 1.2345], [1.0063, 2, 1.0101], [1.2345, 1.0101, 2]]),
        rtol=1e-9,
    )
...
def test_kernel_binet_cauchy():
    ...
    np.testing.assert_allclose(
        kernel,
        np.array([[1, 0.0063, 0.2345], [0.0063, 1, 0.0101], [0.2345, 0.0101, 1]]),
        rtol=1e-9,
    )
...
def test_kernel_gaussian_1d():
    ...
    np.testing.assert_allclose(
        np.matrix.round(gaussian.kernel_matrix, 4),
        np.array(
            [[1.0, 0.26447726, 1.0], [0.26447726, 1.0, 0.26447726], [1, 0.26447726, 1]]
        ),
        atol=1e-04,
    )
    np.testing.assert_allclose(gaussian.kernel_parameter, 2, rtol=1e-4)
...
def test_kernel_gaussian_2d():
    ...
    np.testing.assert_allclose(
        gaussian.kernel_matrix,
        np.array(
            [
                [1.0, 0.39434829, 0.15306655],
                [0.39434829, 1.0, 0.06422136],
                [0.15306655, 0.06422136, 1.0],
            ]
        ),
        atol=1e-4,
    )

    np.testing.assert_allclose(gaussian.kernel_parameter, 1.0, rtol=1e-4)
...
def test_kernel():
    ...
    np.testing.assert_allclose(kernel.kernel_matrix[0, 1], 6.0, rtol=1e-9)
