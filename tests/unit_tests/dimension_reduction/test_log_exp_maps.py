import numpy as np
from UQpy.dimension_reduction.grassmann_manifold.projections.SVDProjection import (
    SVDProjection,
)
from UQpy.dimension_reduction.grassmann_manifold.GrassmannOperations import (
    GrassmannOperations,
)
import sys
...
def test_log_exp_maps():
    sol0 = np.array(
        [
            [0.61415, 1.03029, 1.02001, 0.57327, 0.79874, 0.73274],
    ...
    sol1 = np.array(
        [
            [1.05134, 1.37652, 0.95634, 0.85630, 0.47570, 1.22488],
    ...
    sol2 = np.array(
        [
            [1.04142, 0.91670, 1.47962, 1.23350, 0.94111, 0.61858],
    ...
    sol3 = np.array(
        [
            [0.60547, 0.11492, 0.78956, 0.13796, 0.76685, 0.41661],
    ...
    points_tangent = GrassmannOperations.log_map(
        grassmann_points=manifold_projection.u, reference_point=manifold_projection.u[0]
    )

    np.testing.assert_allclose(points_tangent[0][0][0], 0.0, rtol=1e-2, atol=1e-8)
    np.testing.assert_allclose(points_tangent[1][0][0], 0.0, rtol=1e-8, atol=1e-8)
    np.testing.assert_allclose(points_tangent[2][0][0], 0.0, rtol=1e-8, atol=1e-8)
    np.testing.assert_allclose(points_tangent[3][0][0], 0.0, rtol=1e-8, atol=1e-8)

    manifold_points = GrassmannOperations.exp_map(
        tangent_points=points_tangent, reference_point=manifold_projection.u[0]
    )

    np.testing.assert_allclose(manifold_points[0].data[0][0], -0.41808, rtol=1e-5)
    np.testing.assert_allclose(manifold_points[1].data[0][0], -0.4180759, rtol=1e-8)
    np.testing.assert_allclose(manifold_points[2].data[0][0], -0.4180759, rtol=1e-8)
    np.testing.assert_allclose(manifold_points[3].data[0][0], -0.4180759, rtol=1e-8)
