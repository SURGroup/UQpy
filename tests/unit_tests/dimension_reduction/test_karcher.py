...
def test_karcher_mean_2point():
    ...
    np.testing.assert_allclose(mean.data, solution, atol=1e-8)


def test_karcher_mean_4point():
    """Test karcher mean on 4 points in the manifold Gr(1, 2)"""
    points = [
        np.array([[np.cos(np.deg2rad(theta))], [np.sin(np.deg2rad(theta))]])
        for theta in (0, 15, 75, 90)
    ]
    mean = GrassmannOperations.karcher_mean(
        grassmann_points=points,
        optimization_method="GradientDescent",
    ...
    solution = np.array([[np.cos(np.deg2rad(45))], [np.sin(np.deg2rad(45))]])
    np.testing.assert_allclose(mean.data, solution)
