...
@given(
    ...
    n_x=st.integers(min_value=64, max_value=128),
    modes=st.integers(min_value=1, max_value=32),
)
def test_output_shape(n_batch, in_channels, out_channels, n_x, modes):
    ...
    y = spectral_conv(x)
    assert y.shape == torch.Size([n_batch, out_channels, n_x])
