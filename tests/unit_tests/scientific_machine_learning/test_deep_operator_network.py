...
def test_1d_output_shape(x_resolution, n_samples):
    ...
    model = DeepOperatorNetwork(nn.Linear(x_resolution, 7), nn.Linear(x_dimension, 7))
...
def test_2d_output_shape(x_resolution, n_samples):
    ...
    model = DeepOperatorNetwork(nn.Linear(x_resolution, 10), nn.Linear(x_dimension, 10))
...
def test_no_bias():
    ...
    model = DeepOperatorNetwork(
        nn.Linear(x_resolution, 7, bias=False), nn.Linear(1, 7, bias=False)
    )
