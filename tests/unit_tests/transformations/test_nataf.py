...
def test_non_identity_correlation_uniform_z():
    ...
    rx = np.array([[1.0, 0.8], [0.8, 1.0]])
    ntf_obj = Nataf(distributions=[dist1, dist2], corr_x=rx)
    expected_corr_z = np.array([[1.0, 0.8134732861515996], [0.8134732861515996, 1.0]])
    assert np.allclose(ntf_obj.corr_z, expected_corr_z)
...
def test_non_identity_correlation_uniform_x():
    ...
    expected_corr_x = np.array([[1.0, 0.7859392826067285], [0.7859392826067285, 1.0]])
    assert np.allclose(ntf_obj.corr_x, expected_corr_x)
...
def test_attribute_h():
    ...
    rz = np.array([[1.0, 0.8], [0.8, 1.0]])
    ntf_obj = Nataf(distributions=[dist1, dist2], corr_z=rz)
    expected_H = np.array([[1.0, 0.0], [0.8, 0.6]])
    assert np.allclose(ntf_obj.H, expected_H)
...
def test_samples_x_jxz2():
    ...
    assert np.allclose(ntf_obj.jxz, expected_jxz)
...
def test_samples_x1():
    ...
    samples_x = np.array([[0.3, 1.2, 3.5], [0.2, 2.4, 0.9]]).T
    ntf_obj.run(samples_x=samples_x, jacobian=True)
    expected_samples_z = np.array(
        [
            [-1.5547735945968535, -1.501085946044025],
            [-0.7063025628400874, 0.841621233572914],
            [0.5244005127080407, -0.5244005127080409],
        ]
    )
    assert np.allclose(ntf_obj.samples_z, expected_samples_z)
...
def test_samples_z_jzx2():
    ...
    assert np.allclose(ntf_obj.jzx, expected_jzx)
...
def test_samples_z2():
    ...
    samples_z = np.array([[0.3, 1.2], [0.2, 2.4]]).T
    ntf_obj.run(samples_z=samples_z, jacobian=True)
    expected_samples_x = np.array(
        [
            [3.089557110944763, 1.737779128317309],
            [4.424651648891459, 2.9754073922262116],
        ]
    )
    assert np.allclose(ntf_obj.samples_x, expected_samples_x)
