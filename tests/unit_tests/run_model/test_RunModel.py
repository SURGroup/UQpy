...
def test_append_samples_false():
    ...
    assert np.allclose(
        np.array(model_python_serial_class.qoi_list).flatten(),
        np.sum(x_mcs_new.samples, axis=1),
    )
...
def test_python_serial_workflow_function_vectorized():
    ...
    model_python_serial_function = RunModel(model=model, samples=None)
...
def test_python_parallel_workflow_class():
    ...
    assert np.allclose(
        np.array(model_python_parallel_class.qoi_list).flatten(),
        np.sum(x_mcs.samples, axis=1),
    )
...
def test_third_party_parallel():
    ...
    model = ThirdPartyModel(
        model_script="python_model_sum_scalar.py",
        fmt="{:>10.4f}",
        delete_files=True,
    ...
    m.run(x_mcs.samples)
    assert np.allclose(
        np.array(m.qoi_list).flatten(), np.sum(x_mcs.samples, axis=1), atol=1e-4
    )
    shutil.rmtree(m.model.model_dir)
...
def test_third_party_default_var_names():
    ...
    model_third_party_default_names = RunModel(
        model=model, ntasks=3, samples=x_mcs.samples
    )
    assert np.allclose(
        np.array(model_third_party_default_names.qoi_list).flatten(),
        np.sum(x_mcs.samples, axis=1),
        atol=1e-4,
    )
...
def test_python_serial_workflow_function_wrong_object_name():
    with pytest.raises(AttributeError):
        model = PythonModel(
            model_script="python_model.py", model_object_name="random_model_name"
        )
        model = RunModel(model=model)
        model.run(x_mcs.samples)
...
def test_run_model_none_samples():
    model = PythonModel(model_script="python_model.py", model_object_name="sum_rvs")
    model_python_serial_function = RunModel(model=model, samples=None)
