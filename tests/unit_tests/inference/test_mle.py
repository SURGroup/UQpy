...
def test_simple_probability_model():
    ...
    ml_estimator = MLE(
        inference_model=candidate_model, data=data_1, n_optimizations=3, random_state=1
    )

    np.testing.assert_allclose(ml_estimator.mle[0], 0.003881247615960185)
    np.testing.assert_allclose(ml_estimator.mle[1], 0.09810041339322118)
...
def test_regression_model():
    ...
    noise = (
        Normal(loc=0.0, scale=np.sqrt(error_covariance))
        .rvs(nsamples=4, random_state=1)
        .reshape((4,))
    )
    data_3 = data_clean + noise

    candidate_model = ComputationalModel(
        n_parameters=2, runmodel_object=h_func, error_covariance=error_covariance
    )
    ...
    np.testing.assert_allclose(ml_estimator.mle[0], 0.8689097631871134)
    np.testing.assert_allclose(ml_estimator.mle[1], 2.0030767805841143)
