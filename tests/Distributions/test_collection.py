...
def test_beta():
    assert Beta(a=1.0, b=2.0).cdf(x=0.8) == 0.96
...
def test_chi_square():
    assert np.round(ChiSquare(df=5.0).cdf(x=0.8), 3) == 0.023
...
def test_gamma():
    assert np.round(Gamma(a=2.0).cdf(x=0.8), 3) == 0.191


def test_gen_extreme():
    assert GenExtreme(c=2.0).cdf(x=0.8) == 1.0


def test_inverse_gauss():
    assert np.round(InvGauss(mu=2.0).cdf(x=0.8), 3) == 0.411
...
def test_lognormal():
    assert np.round(Lognormal(s=2.0).cdf(x=0.8), 3) == 0.456
...
def test_pareto():
    assert np.round(Pareto(b=2.0).cdf(x=1.1), 3) == 0.174


def test_poisson():
    assert np.round(Poisson(mu=2.0).cdf(x=1.0), 3) == 0.406
...
def test_truncated_normal():
    assert np.round(TruncNorm(a=-1.0, b=1.0).cdf(x=0.8), 3) == 0.922
...
def test_multinomial_2():
    assert (
        np.round(Multinomial(n=5, p=[0.2, 0.3, 0.5]).log_pmf(x=[1, 1, 3]), 3) == -1.897
    )
...
def test_multinomial_4():
    assert np.all(
        Multinomial(n=5, p=[0.2, 0.3, 0.5]).moments(moments2return="m")
        == [1.0, 1.5, 2.5]
    )
...
def test_multinomial_6():
    ...
    assert np.all(moments[0] == true_values[0]) and np.all(
        np.round(moments[1], 2) == true_values[1]
    )


def test_mvnormal_1():
    assert np.round(MVNormal(mean=[1.0, 2.0], cov=3.0).cdf(x=[0.8, 0.8]), 3) == 0.111


def test_mvnormal_2():
    assert np.round(MVNormal(mean=[1.0, 2.0], cov=3.0).pdf(x=[0.8, 0.8]), 3) == 0.041


def test_mvnormal_3():
    assert (
        np.round(MVNormal(mean=[1.0, 2.0], cov=3.0).log_pdf(x=[0.8, 0.8]), 3) == -3.183
    )
...
def test_mvnormal_4():
    ...
    assert np.all(dict_fit["mean"] == true_mean) and np.all(
        np.round(dict_fit["cov"], 3) == true_cov
    )


def test_mvnormal_5():
    samples = MVNormal(mean=[1.0, 2.0], cov=1.0).rvs(nsamples=3, random_state=123)
    assert np.all(
        np.round(samples, 3)
        == np.array([[-0.086, 2.997], [1.283, 0.494], [0.421, 3.651]])
    )


def test_mvnormal_6():
    assert np.all(
        MVNormal(mean=[1.0, 2.0], cov=3.0).moments(moments2return="m") == [1.0, 2.0]
    )


def test_mvnormal_7():
    assert np.all(MVNormal(mean=[1.0, 2.0], cov=3.0).moments(moments2return="v") == 3.0)


def test_mvnormal_8():
    moments = MVNormal(mean=[1.0, 2.0], cov=3.0).moments(moments2return="mv")
    assert np.all(moments[0] == [1.0, 2.0]) and moments[1] == 3.0
...
def test_clayton():
    assert np.round(Clayton(theta=2.0).evaluate_cdf(unif=unif), 3) == 0.393


def test_frank():
    assert np.round(Frank(theta=2.0).evaluate_cdf(unif=unif), 3) == 0.379


def test_gumbel_1():
    assert np.round(Gumbel(theta=2.0).evaluate_cdf(unif=unif), 3) == 0.398


def test_gumbel_2():
    assert np.round(Gumbel(theta=2.0).evaluate_pdf(unif=unif), 3) == 0.261
...
def test_joint_ind_1():
    ...
    dist_joint_.update_params(loc_0=3.0)
    assert dist_joint_.get_params()["loc_0"] == 3.0
...
def test_joint_ind_6():
    assert np.all(np.round(dist_joint.moments(moments2return="m"), 3) == [2.0, 4.482])
...
def test_joint_copula_1():
    ...
    dist_joint_.update_params(theta_c=2.0)
    assert dist_joint_.get_params()["theta_c"] == 2.0
