...
def test_get_params():
    assert dist_continuous.get_params()["loc"] == 1.0


def test_update_params():
    dist = Uniform(loc=1.0, scale=2.0)
    dist.update_params(loc=2.0)
    assert dist.get_params()["loc"] == 2.0
...
def test_update_params_copula():
    ...
    copula.update_params(theta=1.0)
    assert copula.get_params()["theta"] == 1.0
