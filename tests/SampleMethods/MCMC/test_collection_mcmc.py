...
#Maybe delete
# def test_mmh_2d_joint_symmetric_proposal():
#     target = Distributions.MVNormal([0., 0.]).pdf
#     proposal = [Distributions.Normal(scale=0.2), Distributions.Normal(scale=0.2)]
#     x = MMH(dimension=2, pdf_target=target, nsamples=10, nchains=1, proposal=proposal, proposal_is_symmetric=True,
#             random_state=123)
#     assert [round(float(x.samples[-1][0]), 3), round(float(x.samples[-1][1]), 3)] == [-0.783, -0.195]
...
# def test_mmh_2d_savelogpdf():
#     target = Distributions.MVNormal([0., 0.]).pdf
#     proposal = [Distributions.Normal(scale=0.2), Distributions.Normal(scale=0.2)]
#     x = MMH(dimension=2, pdf_target=target, nsamples=10, nchains=1, proposal=proposal, save_log_pdf=True,
#             random_state=123)
#     assert round(float(x.log_pdf_values[-1]), 3) == -2.163
