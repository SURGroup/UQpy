...
The geodesic distance :math:`d_{\mathcal{G}(p,n)}\left(\mathbf{\Psi}_0,\mathbf{\Psi}_1\right)` between two points
on $\mathcal{G}(p,n)$ corresponds to the distance over the geodesic :math:`\gamma(t)` and it is given by

.. math:: d_{\mathcal{G}(p,n)}\left(\mathbf{\Psi}_0,\mathbf{\Psi}_1\right) = ||\mathbf{\Theta}||_2

where :math:`\mathbf{\Theta} = \left(\theta_1, \theta_2, \dots, \theta_p \right)` contains the principal angles.
Several definitions of distance on :math:`\mathcal{G}(p,n)` can be found in the literature.
...
and :math:`c_i \in \mathbb{R}`. Further, the Grassmann kernel can be defined as a function
:math:`k:\mathcal{G}(p,n)\times \mathcal{G}(p,n) \rightarrow \mathbb{R}` if it is well-defined and positive definite [6]_.
