...
Extension of the stratified sampling capabilities in ``UQpy`` can be performed through subclassing from the three main classes.
First, the user can define a new geometric decomposition of the parameter space by creating a new subclass of the ``Strata`` class.
To implement a new stratified sampling method based on a new stratification, the user must write a new subclass of the ``Strata`` class defining the new decomposition.
