...
from UQpy.scientific_machine_learning.trainers.PyroBBBTrainer import mean_field_guide_factory, TraceMeanField_ELBO_experimental
...
guide = mean_field_guide_factory(model, posterior_mu_initial = (0.0, 0.1), posterior_rho_initial = (-3.0, 0.1))

dataset = SinusoidalDataset(n_samples=n_samples)
train_data = DataLoader(dataset, batch_size=20, shuffle=True)
optimizer = pyro.optim.Adam({"lr": 1e-3})
trainer = sml.PyroBBBTrainer(model, optimizer, guide=guide, loss_function=TraceMeanField_ELBO_experimental)
