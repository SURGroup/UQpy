...
class PyroBNN(PyroModule):
    def __init__(self, width, prior_scale):
        ...
        self.sigma = PyroSample(dist.Gamma(0.1, 4)) # Using SVI, we can also learn the noise level
        # self.sigma = 0.05
...
trainer = sml.PyroBBBTrainer(model, optimizer, guide=guide, loss_function=Trace_ELBO)
print("Starting Training...", end="")
trainer.run(train_data=train_data, epochs=10_000, num_samples=10)
print("done")
...
n = 1_000
predictive = Predictive(model, guide=guide, num_samples=n, return_sites=("_RETURN","sigma"))
