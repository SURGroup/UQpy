...
class Voronoi(Strata):
    """
        ...
        ``Voronoi`` is a child class of the ``Strata`` class.
        ...
        * **dimension** (`ndarray`):
            The dimension of the unit hypercube in which to generate random seeds. Used only if `seeds_number` is
            provided. The user must provide `seeds` or `seeds_number` and `dimension`
        ...
        * **seeds** (`ndarray`):
            ...
            coordinates of the point inside each stratum that defines the stratum.
            If :math:`decomposition_iterations > 1` the `seeds` attribute will differ from the `seeds` input due to the
            iterations.
        ...
        * **voronoi** (`object` of ``scipy.spatial.Voronoi``)
            ...
            the unit hypercube, the code reflects the points on the unit hypercube across all faces of the unit
            hypercube. This causes the Voronoi decomposition to create edges along the faces of the hypercube.
        ...
        * **volume** (`ndarray`):
            An array of dimension `(strata_number, )` containing the volume of each Voronoi stratum in the unit
            hypercube.
        ...
        for j in range(self.mesh.nsimplex):
            try:
                ...
                (
                    ...
                    self.mesh.volumes[j],
                ) = Delaunay.compute_delaunay_centroid_volume(
    ...
    def estimate_gradient(
        ...
        if (
            ...
            dy_dx[update_array, :] = self._estimate_gradient(
    ...
    def _generate_sample(self, bin_, random_state):
        ...
        * **new** (`ndarray`):
        ...
        new = SimplexSampling(
            nodes=self.mesh.sub_simplex, samples_number=1, random_state=random_state
        ).samples
        return new
