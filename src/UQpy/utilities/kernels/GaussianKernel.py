...
class GaussianKernel(EuclideanKernel):
    ...
    def optimize_parameters(
        ...
        cut_off = self._estimate_cut_off(
            data, n_nearest_neighbors, n_cutoff_samples, random_state
        )
    ...
    def _estimate_cut_off(self, data, n_nearest_neighbors, n_partition, random_state):
        ...
        if n_partition is not None:
            ...
            distance_matrix = sd.cdist(
                data[random_indices[:n_partition]], data, metric="euclidean"
            )
        else:
