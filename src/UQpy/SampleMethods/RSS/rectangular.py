    ...
    **Methods:**
    ...
    def __init__(
        ...
        if not isinstance(sample_object, RectangularSTS):
            raise NotImplementedError(
                "UQpy Error: sample_object must be an object of the RectangularSTS class."
            )
    ...
    def _gerss(self):
        ...
        for i in range(self.samples.shape[0], self.nsamples, self.n_add):
            ...
            if (
                ...
                dy_dx[:i] = self.estimate_gradient(
                    np.atleast_2d(self.training_points),
                    np.atleast_2d(np.array(qoi)),
                    self.strata_object.seeds + 0.5 * self.strata_object.widths,
                )
            else:
                ...
                neighbors = knn.kneighbors(
                    np.atleast_2d(self.training_points[-1]), return_distance=False
                )

                # Recompute the gradient only at the nearest neighbor points.
                dy_dx[neighbors] = self.estimate_gradient(
                    np.squeeze(self.training_points[neighbors]),
                    np.array(qoi)[neighbors][0],
                    np.squeeze(
                        self.strata_object.seeds[neighbors]
                        + 0.5 * self.strata_object.widths[neighbors]
                    ),
                )
            ...
            for j in range(i):
                s[j] = (
                    np.sum(dy_dx1[j, :] * var[j, :] * dy_dx1[j, :])
                    * self.strata_object.volume[j] ** 2
                )
            ...
            self.runmodel_object.run(
                samples=np.atleast_2d(self.samples[-self.n_add :]), append_samples=True
            )
    ...
    def _update_stratum_and_generate_sample(self, bin_):
        ...
        self.strata_object.widths[bin_, dir2break] = (
            self.strata_object.widths[bin_, dir2break] / 2
        )
        self.strata_object.widths = np.vstack(
            [self.strata_object.widths, self.strata_object.widths[bin_, :]]
        )
        self.strata_object.seeds = np.vstack(
            [self.strata_object.seeds, self.strata_object.seeds[bin_, :]]
        )
        # print(self.samplesU01[bin_, dir2break], self.strata_object.seeds[bin_, dir2break] + \
        #       self.strata_object.widths[bin_, dir2break])
        if (
            self.samplesU01[bin_, dir2break]
            < self.strata_object.seeds[bin_, dir2break]
            + self.strata_object.widths[bin_, dir2break]
        ):
            self.strata_object.seeds[-1, dir2break] = (
                self.strata_object.seeds[bin_, dir2break]
                + self.strata_object.widths[bin_, dir2break]
            )
            # print("retain")
        ...
        self.strata_object.volume = np.append(
            self.strata_object.volume, self.strata_object.volume[bin_]
        )

        # Add a uniform random sample inside the new stratum
        new = stats.uniform.rvs(
            loc=self.strata_object.seeds[-1, :],
            scale=self.strata_object.widths[-1, :],
