...
class DREAM(MCMC):
    ...
    **Methods:**
    ...
    def __init__(
        ...
        for key, typ in zip(
            ["delta", "c", "c_star", "n_cr", "p_g"], [int, float, float, int, float]
        ):
    ...
    def run_one_iteration(self, current_state, current_log_pdf):
        ...
        """
        r_diff = np.array(
            [np.setdiff1d(np.arange(self.nchains), j) for j in range(self.nchains)]
        )
        ...
        unif_rvs = (
            Uniform()
            .rvs(
                nsamples=self.nchains * (self.nchains - 1),
                random_state=self.random_state,
            )
            .reshape((self.nchains - 1, self.nchains))
        )
        ...
        multi_rvs = Multinomial(n=1, p=[1.0 / self.delta,] * self.delta).rvs(
        ...
        z = (
            Uniform()
            .rvs(nsamples=self.nchains * self.dimension, random_state=self.random_state)
            .reshape((self.nchains, self.dimension))
        )
        subset_a = [
        ...
        norm_vars = (
            ...
            .rvs(nsamples=self.nchains ** 2, random_state=self.random_state)
            .reshape((self.nchains, self.nchains))
        )
        for j in range(self.nchains):
            for i in subset_a[j]:
        ...
        if (
            self.niterations < self.adapt_cr[0]
            and self.niterations % self.adapt_cr[1] == 0
        ):
        ...
        if (
            (self.nsamples >= 100)
            and (self.niterations < self.check_chains[0])
            and (self.niterations % self.check_chains[1] == 0)
        ):
