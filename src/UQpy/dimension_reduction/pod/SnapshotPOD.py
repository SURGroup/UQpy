...
class SnapshotPOD(POD):
    ...
    def _calculate_reduced_and_reconstructed_solutions(
        ...
        reconstructed_solutions_ = np.dot(
            phi[:, : self.modes], phi_s[:, : self.modes].T
        )
        ...
        for i in range(snapshot_number):
            reconstructed_solutions[0:rows, 0:columns, i] = reconstructed_solutions_[
                i, :
            ].reshape((rows, columns))

        for i in range(self.modes):
            reduced_solutions[0:rows, 0:columns, i] = reduced_solutions_[i, :].reshape(
                (rows, columns)
            )
