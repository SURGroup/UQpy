    ...
    def interpolate_sample(self, coordinates, samples, point, element_wise=True):
        ...
        if element_wise:
                ...
                for k in range(ncols):
                    ...
                    if val_data.count(val_data[0]) == len(val_data):
                        ...
                        if isinstance(self.interpolation_method, Surrogate):
                            ...
                            y = self.interpolation_method.predict(point, return_std=False)
                        else:
                            y = self.interpolation_method.interpolate(coordinates, samples, point)
            ...
            if isinstance(self.interpolation_method, Surrogate):
                ...
                interp_point = self.interpolation_method.interpolate(coordinates, samples, point)
