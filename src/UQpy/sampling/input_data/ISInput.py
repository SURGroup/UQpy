...
class ISInput(SamplingInput):
    ...
    :param callable pdf_target: Callable that evaluates the pdf of the target distribution.
     Either log_pdf_target or pdf_target must be specified (the former is preferred).
    :param callable log_pdf_target: Callable that evaluates the log-pdf of the target distribution.
     Either log_pdf_target or pdf_target must be specified (the former is preferred).
    :param tuple args_target: Positional arguments of the target log_pdf / pdf callable.
    :param Distribution proposal: Proposal to sample from. This UQpy.Distributions object must have an rvs method and a
