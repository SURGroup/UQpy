...
from UQpy.sampling.refined_stratified_sampling.baseclass.RefinedStratifiedSampling import RefinedStratifiedSampling
from UQpy.sampling.SimplexSampling import SimplexSampling
from UQpy.utilities.strata.Voronoi import Voronoi
...
class VoronoiRefinedStratifiedSampling(RefinedStratifiedSampling):
    ...
    ``VoronoiRSS`` is a child class of ``refined_stratified_sampling``. ``VoronoiRSS`` takes in all parameters defined in the
      parent ``refined_stratified_sampling`` class with differences note below. Only those inputs and attributes that differ from
      the parent class are listed below. See documentation for ``refined_stratified_sampling`` for additional details.

    **Inputs:**

    * **sample_object** (``sampling`` object):
        The `sample_object` for ``VoronoiRSS`` can be an object of any ``sampling`` class that possesses the
        following attributes: `samples` and `samplesU01`

        This can be any ``sampling`` object because ``VoronoiRSS`` creates its own `strata_object`. It does not
        use a `strata_object` inherited from an ``stratified_sampling`` object.
    ...
    **Methods:**
    ...
    def __init__(self, sample_object=None, runmodel_object=None, kriging=None, update_locally=False,
        ...
        if hasattr(sample_object, 'samplesU01'):
            self.strata_object = Voronoi(seeds=sample_object.samplesU01)
    ...
    def run_refined_stratified_sampling(self):
        ...
        the ``refined_stratified_sampling`` class for additional details.
        """
