...
class RefinedStratifiedSampling:
    ...
    * **sample_object** (``sampling`` object(s)):
        Generally, this must be an object of a ``UQpy.sampling`` class. Each child class of ``refined_stratified_sampling`` has it's
        own constraints on which specific types of ``sampling`` it can accept. These are described in the child
        class documentation below.

    * **runmodel_object** (``RunModel`` object):
        A ``RunModel`` object, which is used to evaluate the model.

        `runmodel_object` is optional. If it is provided, the specific ``refined_stratified_sampling`` subclass with use it to compute the
        gradient of the model in each stratum for gradient-enhanced refined stratified sampling. If it is not
        provided, the ``refined_stratified_sampling`` subclass will default to random stratum refinement.
    ...
    * **nsamples** (`int`):
        ...
        `nsamples` is not provided, an ``refined_stratified_sampling`` subclass can be executed by invoking the ``run`` method and passing
        `nsamples`.
    ...
    **Attributes:**
    ...
    def __init__(self, sample_object=None, runmodel_object=None, kriging=None, update_locally=False,
        ...
        if runmodel_object is not None:
            ...
            if self.verbose:
                print('UQpy: GE-refined_stratified_sampling - Running the initial sample set.')
            self.runmodel_object.run(samples=self.samples)
            if self.verbose:
                print('UQpy: GE-refined_stratified_sampling - A refined_stratified_sampling class object has been initiated.')
        else:
            if self.verbose:
                print('UQpy: refined_stratified_sampling - A refined_stratified_sampling class object has been initiated.')
    ...
    def run(self, nsamples):
        """
        Execute the random sampling in the ``refined_stratified_sampling`` class.

        The ``run`` method is the function that performs random sampling in any ``refined_stratified_sampling`` class. If `nsamples` is
        provided, the ``run`` method is automatically called when the ``refined_stratified_sampling`` object is defined. The user may also call
        the ``run`` method directly to generate samples. The ``run`` method of the ``refined_stratified_sampling`` class can be invoked many
        ...
        calling a ``run_rss`` method that is uniquely defined for each subclass. All ``refined_stratified_sampling`` subclasses must posses a
        ...
        **Output/Return:**
        ...
        `strata_object` attributes of the ``refined_stratified_sampling`` class.
        """
