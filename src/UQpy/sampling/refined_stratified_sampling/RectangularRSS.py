...
from UQpy.sampling.stratified_sampling import RectangularSTS
import numpy as np
import scipy.stats as stats
import copy


class RectangularRSS(RefinedStratifiedSampling):
    """
    Executes Refined Stratified Sampling using Rectangular Stratification.

    ``RectangularRSS`` is a child class of ``refined_stratified_sampling``. ``RectangularRSS`` takes in all parameters defined
      in the parent ``refined_stratified_sampling`` class with differences note below. Only those inputs and attributes that
      differ from the parent class are listed below. See documentation for ``refined_stratified_sampling`` for additional details
    ...
    **Methods:**
    ...
    def _generate_gradient_enhanced_samples(self):
        ...
        """
        if self.verbose:
            print('UQpy: Performing GE-refined_stratified_sampling with rectangular stratification...')
    ...
    def _generate_samples(self):
        ...
        if self.verbose:
            print('UQpy: Performing refined_stratified_sampling with rectangular stratification...')
