...
class RectangularSTS(StratifiedSampling):
    ...
    ``RectangularSTS`` is a child class of ``stratified_sampling``. ``RectangularSTS`` takes in all parameters defined in the parent
    ...
    **Methods:**
    ...
    def __init__(self, distributions, strata_object, samples_per_stratum_number=None, samples_number=None, sts_criterion="random",
        ...
        if not isinstance(strata_object, Rectangular):
