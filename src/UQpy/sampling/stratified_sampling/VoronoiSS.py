...
class VoronoiStratification(StratifiedSampling):
    ...
    ``VoronoiSTS`` is a child class of ``stratified_sampling``. ``VoronoiSTS`` takes in all parameters defined in the parent
    ``stratified_sampling`` class with differences note below. Only those inputs and attributes that differ from the parent class are
    listed below. See documentation for ``stratified_sampling`` for additional details.
    ...
    **Methods:**
    ...
    def __init__(self, distributions, strata_object, samples_per_stratum_number=None, samples_number=None,
        ...
        # Check strata_object
        if not isinstance(strata_object, Voronoi):
