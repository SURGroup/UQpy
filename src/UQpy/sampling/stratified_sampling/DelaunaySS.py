...
class DelaunaySTS(StratifiedSampling):
    ...
    ``DelaunaySTS`` is a child class of ``stratified_sampling``. ``DelaunaySTS`` takes in all parameters defined in the
      parent ``stratified_sampling`` class with differences note below. Only those inputs and attributes that differ from
      the parent class are listed below. See documentation for ``stratified_sampling`` for additional details.
    ...
    **Methods:**
    ...
    def __init__(self, distributions, strata_object, samples_per_stratum_number=1, samples_number=None,
        ...
        if not isinstance(strata_object, Delaunay):
    ...
    def create_unit_hypercube_samples(self, samples_per_stratum_number=None, samples_number=None):
        ...
        the ``stratified_sampling`` class for additional details.
        """
    ...
    def extend_weights(self, index, weights):
        if int(self.samples_per_stratum_number[index]) != 0:
            ...
            weights.extend([0] * int(self.samples_per_stratum_number[index]))
