...
class DRAM(MCMC):
    ...
    def __init__(
        ...
        **References:**
        ...
        super().__init__(
            ...
            args_target=dram_input.args_target,
            dimension=dram_input.dimension,
            seed=dram_input.seed,
            burn_length=dram_input.burn_length,
            jump=dram_input.jump,
            save_log_pdf=dram_input.save_log_pdf,
            concatenate_chains=dram_input.concatenate_chains,
            random_state=dram_input.random_state,
            chains_number=dram_input.chains_number,
        )
        ...
        self.sample_covariance = np.zeros(
            (self.chains_number, self.dimension, self.dimension)
        )
    ...
    def run_one_iteration(self, current_state, current_log_pdf):
        ...
        multivariate_normal = MultivariateNormal(
            mean=np.zeros(self.dimension,), cov=1.0
        )
        ...
        delayed_chains_indices = (
            []
        )  # indices of chains that will undergo delayed rejection
        unif_rvs = (
            Uniform()
        ...
        for nc, (cand, log_p_cand, log_p_curr) in enumerate(
            ...
            if accept:
                ...
                delayed_chains_indices.append(
                    nc
                )  # these indices will enter the delayed rejection part

        # Delayed rejection
        if (
            len(delayed_chains_indices) > 0
        ):  # performed delayed rejection for some chains
            ...
            for i, nc in enumerate(delayed_chains_indices):
                ...
                candidate2[i, :] = current_states_delayed[
                    i, :
            ...
            log_prop_cand_cand2 = multivariate_normal.log_pdf(
                candidates_delayed - candidate2
            )
            log_prop_cand_curr = multivariate_normal.log_pdf(
            ...
            unif_rvs = (
                Uniform()
                .rvs(
                    nsamples=len(delayed_chains_indices), random_state=self.random_state
                )
                .reshape((-1,))
            )
        ...
        for nc in range(self.chains_number):
            ...
            if (self.iterations_number > 1) and (
                self.iterations_number % self.covariance_update_rate == 0
            ):
                self.current_covariance[
                    nc
                ] = self.scale_parameter * self.sample_covariance[nc] + 1e-6 * np.eye(
                    self.dimension
                )
        if self.save_covariance and (
            (self.iterations_number > 1)
            and (self.iterations_number % self.covariance_update_rate == 0)
        ):
    ...
    def _recursive_update_mean_covariance(
        ...
        * new_covariance (ndarray (dim, dim)): Updated sample covariance
        ...
        new_mean = (
            samples_number - 1
        ) / samples_number * previous_mean + 1 / samples_number * new_sample
        ...
        if samples_number == 1:
            ...
            new_covariance = (samples_number - 2) / (
                samples_number - 1
