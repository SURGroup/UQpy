...
class DRAM(MCMC):
    ...
    def __init__(
        ...
        self.sample_covariance = np.zeros(
            (self.n_chains, self.dimension, self.dimension)
        )
    ...
    def run_one_iteration(self, current_state: np.ndarray, current_log_pdf: np.ndarray):
        ...
        for nc in range(self.n_chains):
            ...
            if (self.iterations_number > 1) and (
                ...
                self.current_covariance[nc] = (
                    self.scale_parameter * self.sample_covariance[nc]
