...
class ParallelTemperingMCMC(TemperingMCMC):
    ...
    def __init__(
        self,
        n_iterations_between_sweeps: PositiveInteger,
        ...
        if not isinstance(samplers, list):
            self.samplers = [
                samplers.__copy__() for _ in range(len(tempering_parameters))
            ]
        else:
        ...
        if self.tempering_parameters is None:
            if self.n_tempering_parameters is None:
                raise ValueError(
                    "UQpy: either input tempering_parameters or n_tempering_parameters should be provided."
                )
            elif not (
                isinstance(self.n_tempering_parameters, int)
                and self.n_tempering_parameters >= 2
            ):
                raise ValueError(
                    "UQpy: input n_tempering_parameters should be a integer >= 2."
                )
            else:
                self.tempering_parameters = [
                    1.0 / np.sqrt(2) ** i
                    for i in range(self.n_tempering_parameters - 1, -1, -1)
                ]
        elif (
            not isinstance(self.tempering_parameters, (list, tuple))
            or not (
                all(
                    isinstance(t, (int, float)) and (0 < t <= 1.0)
                    for t in self.tempering_parameters
                )
            )
        ...
        for i, sampler in enumerate(self.samplers):
            if isinstance(sampler, MetropolisHastings) and sampler.proposal is None:
                ...
                self.samplers[i] = sampler.__copy__(
                    proposal_is_symmetric=True,
                    proposal=JointIndependent(
                        [Normal(scale=1.0 / np.sqrt(self.tempering_parameters[i]))]
                        * sampler.dimension
                    ),
                )
        ...
        for i, temper_param in enumerate(self.tempering_parameters):
            log_pdf_target = (
                lambda x, temper_param=temper_param: self.evaluate_log_reference(x)
                + self.evaluate_log_intermediate(x, temper_param)
            )
            self.mcmc_samplers.append(
                self.samplers[i].__copy__(
    ...
    def run(
        ...
        for i, mcmc_sampler in enumerate(self.mcmc_samplers):
            if (
                mcmc_sampler.evaluate_log_target is None
                and mcmc_sampler.evaluate_log_target_marginals is None
            ):
                (
                    mcmc_sampler.evaluate_log_target,
                    mcmc_sampler.evaluate_log_target_marginals,
                ) = mcmc_sampler._preprocess_target(
                    pdf_=mcmc_sampler.pdf_target,
                    log_pdf_=mcmc_sampler.log_pdf_target,
                    args=mcmc_sampler.args_target,
                )
            ns, ns_per_chain, current_state_t, current_log_pdf_t = (
                mcmc_sampler._initialize_samples(
        ...
        while self.mcmc_samplers[0].nsamples_per_chain < final_ns_per_chain:
            ...
            if (
                self.mcmc_samplers[-1].iterations_number
                > self.mcmc_samplers[-1].burn_length
                and (
                    self.mcmc_samplers[-1].iterations_number
                    - self.mcmc_samplers[-1].burn_length
                )
                % self.mcmc_samplers[-1].jump
                == 0
            ):
                for t, sampler in enumerate(self.mcmc_samplers):
                    sampler.samples[sampler.nsamples_per_chain, :, :] = new_state[
                        t
                    ].copy()
                    if self.save_log_pdf:
                        sampler.log_pdf_values[sampler.nsamples_per_chain, :] = (
                            new_log_pdf[t].copy()
                        )
    ...
    @beartype
    def evaluate_normalization_constant(
        self, compute_potential, log_Z0: float = None, nsamples_from_p0: int = None
    ):
        """
        ...
        if not self.save_log_pdf:
            raise NotImplementedError(
                "UQpy: the evidence cannot be computed when save_log_pdf is set to False."
            )
        if log_Z0 is None and nsamples_from_p0 is None:
            raise ValueError(
                "UQpy: input log_Z0 or nsamples_from_p0 should be provided."
            )
        ...
        for i, (temper_param, sampler) in enumerate(
            ...
            potential_values = compute_potential(
                x=sampler.samples,
                temper_param=temper_param,
                log_intermediate_values=log_factor_values,
            )
        ...
        if log_Z0 is None:
            ...
            log_Z0 = np.log(1.0 / nsamples_from_p0) + logsumexp(
                self.evaluate_log_intermediate(
                    x=samples_p0, temper_param=self.tempering_parameters[0]
                )
            )
