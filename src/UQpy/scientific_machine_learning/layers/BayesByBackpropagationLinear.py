...
from UQpy.scientific_machine_learning.losses.metrics import calculate_kl as KL_DIV
from UQpy.scientific_machine_learning.baseclass.Layer import Layer


class BayesByBackpropagationLinear(Layer):
    def __init__(
        self,
        in_features: int,
        out_features: int,
        ...
        :param priors:
        """
        super(BayesByBackpropagationLinear, self).__init__()
    ...
    def forward(self, x: torch.Tensor, sample: bool = True) -> torch.Tensor:
        ...
        if self.training or sample:
            weight_epsilon = torch.empty(self.W_mu.size()).normal_(0, 1).to(self.device)
