...
class SVITrainer:
    ...
    def __init__(
        ...
        """Prepare to train a Bayesian neural network using Stochastic Variational Inference
        ...
        self.loss_function = lambda pred, y, sigma: (pred - y).pow(2).sum() / (2 * sigma ** 2) + torch.log(sigma) * y.numel()
         # Negative log likelihood for Gaussian likelihood with learned sigma
        ...
        while i < epochs and average_train_loss >= tolerance:
            if train_data:
                ...
                for batch_number, (*x, y) in enumerate(train_data):
                    ...
                    sigma_samples = torch.zeros(num_samples)
                    for sample in range(num_samples):
                        prediction = self.model(*x, return_log_prob=True)
                        sigma_samples[sample] = self.model.network.sigma.detach().cpu()
                        nll_loss[sample] = self.loss_function(prediction, y, self.model.network.sigma)
                        log_q = sum(m.qs for m in self.model.network.modules() if hasattr(m, "qs"))
                        log_p = sum(m.ps for m in self.model.network.modules() if hasattr(m, "ps"))
                        kl_loss[sample] = log_q - log_p
                    divergence_loss = torch.mean(kl_loss)
                    mean_nll = torch.mean(nll_loss)
                    train_loss = mean_nll + divergence_loss / len(y)
                    train_loss.backward()
                    self.optimizer.step()
                    ...
                    self.history["sigma"] = torch.mean(sigma_samples)
                if self.scheduler:
            ...
            log_message = (
                ...
                f" Train Sigma {self.history['sigma']:.6e}"
            )
            if test_data:
                total_test_nll = 0
                with torch.no_grad():
