...
def fisher(
    ...
    if metric == Metric.JACOBIAN_DIAG:
        ...
        hess = torch.autograd.functional.hessian(
            log_prob_func, params, create_graph=True
        )
...
>>>>>>> origin/feature/scientific_machine_learning
def cholesky_inverse(fish, momentum):
    ...
    y = torch.linalg.solve_triangular(
        lower, momentum.view(-1, 1), upper=False, unitriangular=False
    )
    fish_inv_p = torch.linalg.solve_triangular(
        lower.t(), y, upper=True, unitriangular=False
    )
    return fish_inv_p
...
def sample(
    ...
    for n in range(num_samples):
        ...
        try:
            ...
            if debug == 1:
                print(
                    "Step: {}, Current Hamiltoninian: {}, Proposed Hamiltoninian: {}".format(
                        n, ham, new_ham
                    )
                )
...
def define_split_model_log_prob(
    model,
    model_loss,
    train_loader,
    num_splits,
...
def sample_model(
    ...
    for weights in model.parameters():
        ...
        if build_tau:
            tau_list.append(torch.tensor(1.0))
...
=======
        ...
        if test_loader.__class__ is torch.utils.data.dataloader.DataLoader:
            ...
            if len(test_loader.dataset) % test_loader.batch_size == 0.0:
                ...
                num_batches = int(
                    round(len(test_loader.dataset) / test_loader.batch_size) + 1
                )

            log_prob_list = define_split_model_log_prob(
                model,
                model_loss,
                test_loader,
                num_batches,
...
>>>>>>> origin/feature/scientific_machine_learning
            ...
            if x.device != samples[0].device:
                raise RuntimeError(
                    "x on device: {} and samples on device: {}".format(
                        x.device, samples[0].device
                    )
                )
            ...
            raise RuntimeError(
                "Val data not defined (i.e. arguments x, y, val_loader are all not defined)"
            )
