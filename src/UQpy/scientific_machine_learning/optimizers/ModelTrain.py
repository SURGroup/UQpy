...
class BaseTrainer:
    ...
    def run_training(self):
        ...
        lr_sched = lr_scheduler.ReduceLROnPlateau(
            optimizer, patience=self.cfg.patience, verbose=True
        )
        ...
        for epoch in range(self.cfg.n_epochs):
            ...
            valid_loss, valid_acc = self.train_model(
                val_loader, optimizer, mode="validate"
            )
...
class BBBTrainer(BaseTrainer):
    ...
    def train_model(self, data_loader, optimizer, mode="train"):
        ...
        mean_kl = torch.mean(
            torch.stack(kl_list)
        ).item()  # Torch for tensors instead of numpy
        self.update_history(
            mode, total_loss / num_batches, total_acc / num_batches, mean_kl
        )
...
class HMCTrainer(BaseTrainer):
    ...
    def _initialize_params(self):
        ...
        if os.path.exists(params_hmc_path):
            ...
            state_dict = torch.load(
                construct_det_path(self.cfg), map_location=self.device
            )
            self.net.load_state_dict(state_dict)
        return hamiltorch.util.flatten(self.net).to(self.device).clone()

    def _save_max_indices(self, max_indices):
        save_path_indices = os.path.join(
            construct_paths(self.cfg)[2], "max_indices.pkl"
        )
    ...
    def _perform_hmc_sampling(self, params_init, max_indices, tau_list):
        test_set, test_loader, out_channels, X_ts, Y_ts, names_ts = load_test_data(
            self.cfg
        )
        ...
        _, train_loader, _, _, _, _, _, _, _, X_tr_tensor, Y_tr_tensor, _, _, _, _ = (
            load_data(self.cfg)
        )
        return hamiltorch.sample_model(
            self.net,
            X_tr_tensor.to(self.device),
            Y_tr_tensor.to(self.device),
            params_init=params_init,
            model_loss=metrics.mse_loss,
            num_samples=self.cfg.num_samples_hmc,
            num_samples_start=start_iter,
            num_samples_end=end_iter,
            burn=self.cfg.burn,
            step_size=self.cfg.step_size,
            num_steps_per_sample=self.cfg.L,
            tau_out=self.cfg.tau_out,
            tau_list=tau_list,
            normalizing_const=self.cfg.normalizing_const,
            store_on_GPU=self.cfg.store_on_GPU,
            sampler=hamiltorch.Sampler.HMC,
            weight_path=construct_paths(self.cfg)[0],
...
class Training:
    def __init__(self, cfg):
        ...
        self.net = self.select_network(
            cfg.method, cfg.nfilters, cfg.kernel_size, cfg.layer_type, drop_rate
        ).to(cfg.device)

    def select_network(
        self, network_type, nfilters, kernel_size, layer_type, drop_rate
    ):
        ...
        if network_type in ["BBB", "BBB_LRT"]:
            ...
            return MonteCarloDropout(
                ...
                kernel_size,
                drop_rate,
                self.cfg.drop_idx_en,
                self.cfg.drop_idx_dec,
            )
        else:
