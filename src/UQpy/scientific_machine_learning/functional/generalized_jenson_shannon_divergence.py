...
@beartype
def generalized_jenson_shannon_divergence(
    ...
    r"""Compute the generalized Jenson-Shannon divergence for a prior and posterior distribution
    ...
    where :math:`D_{KL}` is the Kullback-Leiber divergence and :math:`M=\alpha Q + (1-\alpha) P` is the mixture distribution.
    """
    if len(prior_distributions) != len(posterior_distributions):
        raise RuntimeError("UQpy: `prior_distributions` and `posterior_distributions` must have the same length")
    mc_posterior = MonteCarloSampling(distributions=posterior_distributions, nsamples=num_samples)
    mc_prior = MonteCarloSampling(distributions=prior_distributions, nsamples=num_samples)
    ...
    for i in range(num_samples):
        ...
        for j in range(n_distributions):
            ...
            kl_divergence_q_m = posterior.log_pdf(posterior_samples[j]) - np.log(mixture_pdf_posterior_samples)
    ...
    if reduction == "none":
        ...
        raise ValueError(f"UQpy: Invalid reduction={reduction}. Must be one of 'none', 'mean', or 'sum'")
