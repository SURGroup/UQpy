...
class BispectralRepresentation:
    def __init__(
        ...
        self.biphase: NumpyFloatArray = np.arctan2(
            self.bispectrum_imaginary, self.bispectrum_real
        )
    ...
    def _compute_bicoherence_uni(self):
        ...
        self.ranges = [
            range(self.n_frequency_intervals[i]) for i in range(self.n_dimensions)
        ]
