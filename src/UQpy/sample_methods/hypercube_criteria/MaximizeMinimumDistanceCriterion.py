from UQpy.sample_methods.hypercube_criteria import Criterion, RandomCriterion
from UQpy.sample_methods.hypercube_criteria.DistanceMetricEnum import DistanceMetricEnum
from scipy.spatial.distance import pdist
import numpy as np
import copy


class MaximizeMinimumDistanceCriterion(Criterion):
    """
            ...
            * **lhs_samples** (`ndarray`)
            ...
            """
    def __init__(self, samples, random_state=None, iterations=100, metric=DistanceMetricEnum.EUCLIDEAN):
        self.random_state = random_state
        self.samples = samples
        self.iterations = iterations

        if isinstance(metric, str) & isinstance(metric, DistanceMetricEnum):
