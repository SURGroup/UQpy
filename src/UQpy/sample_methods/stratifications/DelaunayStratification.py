...
class DelaunaySTS(StratifiedSampling):
    ...
    ``DelaunaySTS`` is a child class of ``stratifications``. ``DelaunaySTS`` takes in all parameters defined in the parent
    ...
    **Methods:**
    ...
    """
    def __init__(self, distributions, strata_object, samples_per_stratum_number=1, samples_number=None, random_state=None,
                 verbose=False):
    ...
    def create_unit_hypercube_samples(self, samples_per_stratum_number=None, samples_number=None):
        ...
        for simplex in self.strata_object.delaunay.simplices:  # extract simplices from Delaunay triangulation
            samples_temp = SimplexSampling(nodes=self.strata_object.delaunay.points[simplex],
                                           samples_number=int(self.samples_per_stratum_number[count]), random_state=self.random_state)
            samples_in_strata.append(samples_temp.samples)
            if int(self.samples_per_stratum_number[count]) != 0:
                weights.extend(
                    [self.strata_object.volume[count] / self.samples_per_stratum_number[count]] * int(
