...
class RefinedStratifiedSampling:
    ...
    * **sample_object** (``sample_methods`` object(s)):
        Generally, this must be an object of a ``UQpy.sample_methods`` class. Each child class of ``refined_stratified`` has it's
        own constraints on which specific types of ``sample_methods`` it can accept. These are described in the child
        class documentation below.

    * **runmodel_object** (``RunModel`` object):
        A ``RunModel`` object, which is used to evaluate the model.

        `runmodel_object` is optional. If it is provided, the specific ``refined_stratified`` subclass with use it to compute the
        gradient of the model in each stratum for gradient-enhanced refined stratified sampling. If it is not
        provided, the ``refined_stratified`` subclass will default to random stratum refinement.
    ...
    * **nsamples** (`int`):
        ...
        `nsamples` is not provided, an ``refined_stratified`` subclass can be executed by invoking the ``run`` method and passing
        `nsamples`.
    ...
    **Attributes:**
    ...
    def __init__(self, sample_object=None, runmodel_object=None, kriging=None, update_locally=False,
        ...
        if runmodel_object is not None:
            ...
            if self.verbose:
                print('UQpy: GE-refined_stratified - Running the initial sample set.')
            self.runmodel_object.run(samples=self.samples)
            if self.verbose:
                print('UQpy: GE-refined_stratified - A refined_stratified class object has been initiated.')
        else:
            if self.verbose:
    ...
    def run(self, nsamples):
        """
        Execute the random sampling in the ``refined_stratified`` class.

        The ``run`` method is the function that performs random sampling in any ``refined_stratified`` class. If `nsamples` is
        provided, the ``run`` method is automatically called when the ``refined_stratified`` object is defined. The user may also call
        the ``run`` method directly to generate samples. The ``run`` method of the ``refined_stratified`` class can be invoked many
        ...
        calling a ``run_rss`` method that is uniquely defined for each subclass. All ``refined_stratified`` subclasses must posses a
