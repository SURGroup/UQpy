...
class Distribution1D(Distribution):
    ...
    def _retrieve_1d_data_from_scipy(self, scipy_name=stats.rv_continuous):
        self.cdf = lambda x: scipy_name.cdf(x=self._check_x_dimension(x), **self.parameters)
        self.icdf = lambda x: scipy_name.ppf(q=self._check_x_dimension(x), **self.parameters)
