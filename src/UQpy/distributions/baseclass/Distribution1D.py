...
class Distribution1D(Distribution, ABC):
    ...
    def _retrieve_1d_data_from_scipy(
        self, scipy_name=stats.rv_continuous, is_continuous=True
    ):
        if is_continuous:
            self.cdf = lambda x: scipy_name.cdf(
                x=self.check_x_dimension(x), **self.parameters
            )
        else:
            self.cdf = lambda x: scipy_name.cdf(
                k=self.check_x_dimension(x), **self.parameters
            )
        self.icdf = lambda x: scipy_name.ppf(
            q=self.check_x_dimension(x), **self.parameters
        )
