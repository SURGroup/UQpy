...
class DistributionContinuous1D(Distribution1D):
    ...
    def _construct_from_scipy(self, scipy_name=stats.rv_continuous):
        self.pdf = lambda x: scipy_name.pdf(x=self._check_x_dimension(x), **self.parameters)
        self.log_pdf = lambda x: scipy_name.logpdf(x=self._check_x_dimension(x), **self.parameters)
        self._retrieve_data_from_scipy()
