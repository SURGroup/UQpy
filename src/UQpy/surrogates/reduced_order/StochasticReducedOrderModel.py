...
class StochasticReducedOrderModel:
    ...
    * **samples** (`ndarray`):
        An array/list of samples corresponding to the points at which the reduced_order is defined.
    ...
    **Methods:**
    ...
    def __init__(self, samples, target_distributions, moments=None, weights_errors=None, weights_distribution=None,
        ...
        if self.properties is not None:
            ...
            print('UQpy: No properties list provided, execute the reduced_order by calling run method and specifying a '
                  'properties list')

    def run(self, weights_errors=None, weights_distribution=None, weights_moments=None, weights_correlation=None,
            properties=None):
        """
        Execute the stochastic reduced order model in the ``reduced_order`` class.

        The ``run`` method is the function that computes the probability weights corresponding to the sample. If
        `properties` is provided, the ``run`` method is automatically called when the ``reduced_order`` object is defined. The
        user may also call the ``run`` method directly to generate samples. The ``run`` method of the ``reduced_order`` class can
        ...
        * **weights_correlation** (`ndarray` or `list` of `float`):
        ...
        if self.verbose:
            print('UQpy: Performing reduced_order...')
        ...
        if self.verbose:
            print('UQpy: reduced_order completed!')
