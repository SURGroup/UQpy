...
class TotalDegreeBasis(PolynomialBasis):
    def __init__(
        self,
        ...
        inputs_number = (
            1
            if not isinstance(distributions, (JointIndependent, JointCopula))
            else len(distributions.marginals)
        )
        multi_index_set = PolynomialBasis.calculate_total_degree_set(
            inputs_number=inputs_number, degree=max_degree
        )
        if 0 < hyperbolic < 1:
            mask = (
                np.round(
                    np.sum(multi_index_set**hyperbolic, axis=1) ** (1 / hyperbolic), 4
                )
                <= max_degree
            )
            multi_index_set = multi_index_set[mask]
        polynomials = PolynomialBasis.construct_arbitrary_basis(
            inputs_number, distributions, multi_index_set
        )
        super().__init__(
            inputs_number,
            len(multi_index_set),
