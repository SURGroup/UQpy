    ...
    **Methods:**
    ...
    def run(self, nopt=1, x0=None):
        ...
        * **nopt** (`int`):
        ...
        if (self.inference_model.dist_object is not None) and hasattr(
            self.inference_model.dist_object, "fit"
        ):
        ...
        # Otherwise run optimization
            ...
            if x0 is None:
                ...
                x0 = (
                    Uniform()
                    .rvs(
                        nsamples=nopt * self.inference_model.nparams,
                        random_state=self.random_state,
                    )
                    .reshape((nopt, self.inference_model.nparams))
                )
                if "bounds" in self.kwargs_optimizer.keys():
                    bounds = np.array(self.kwargs_optimizer["bounds"])
                    x0 = (
                        bounds[:, 0].reshape((1, -1))
                        + (bounds[:, 1] - bounds[:, 0]).reshape((1, -1)) * x0
                    )
            else:
                x0 = np.atleast_2d(x0)
            ...
            for x0_ in x0:
                res = self.optimizer(
                    self._evaluate_func_to_minimize, x0_, **self.kwargs_optimizer
                )
                mle_tmp = res.x
